...
5. 输入中的 `0x`、`0X` 前缀需要忽略。
6. 如果一行内容开头是 `0x0000:` 这类“十六进制加冒号”形式，可理解为地址偏移量，这部分要忽略。
7. 支持 `tcpdump -XX` 风格输入，本质上是提取其中的十六进制内容。
8. 普通输入中的杂字符不支持；目标是不再有“解析失败场景”，而不是把任意文本都当作 hex 源。
9. 当提取出的十六进制字符数量为奇数时，自动在末尾补 `0`。
10. “常见 Ethernet 头”只认当前产品需要覆盖的常见类型，不做宽泛的 `EtherType >= 0x0600` 推断。
...
- 如果整行开头匹配 `0x[0-9A-Fa-f]+:`，则该前缀先整体剥离，再处理冒号后面的 token。
...
- 支持把行首 `0x0000:`、`0x0010:` 这类地址偏移前缀视为元数据并忽略。
...
9. 行首 `0x0000:` / `0x0010:` 这类地址偏移前缀会被忽略，不会误当作 payload。
10. 普通 hex 输入中奇数位 token 会自动补 `0`。
11. 普通 hex 输入中的杂字符 token 会被忽略，而不是导致整个 block 失败。
12. `tcpdump -XX` 能忽略描述行，仅提取偏移行 hex 区。
13. `tcpdump -XX` 能忽略右侧 ASCII 区。
14. `tcpdump -XX` 在 `0x0000:` 重置时开始新 packet。
15. `tcpdump -XX` 偏移不连续时仍然输出。
16. 整个输入没有任何可提取 hex 时返回 0 个 packet。
