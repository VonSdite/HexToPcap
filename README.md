...
- 支持 `0x` / `0X` 前缀和行首 `0x0000:` 这类偏移前缀的忽略处理
...
2. 在主窗口粘贴十六进制文本
3. 如需修改输出目录或 Wireshark 路径，点击顶部保存位置右侧的 `设置`
4. 点击 `转换`
...
如果行首是 `0x0000:`、`0x0010:` 这种“十六进制加冒号”的偏移前缀，程序会把它当作地址偏移量忽略，不算报文内容。

### 2. `tcpdump -vv -nn -XX` 输出

程序会识别 `0x0000:` 这类偏移行，把冒号前面的地址偏移量忽略掉，只提取后面的十六进制部分，并忽略右侧 ASCII。
...
当偏移重新回到 `0x0000` 时，会视为一个新报文开始。偏移不连续、跳变或不是从 `0x0000` 开始时，也不会阻止导出。

## 解析规则

- 空行会被视为显式分包边界
- 普通十六进制输入支持忽略 `0x` / `0X` 前缀，以及行首 `0x0000:` 这类偏移前缀
...
- 奇数长度 token 自动补 `0`
- 不完整报文片段的直接导出
- `tcpdump -XX` 输入解析、ASCII 忽略与偏移跳变处理
- `pcap` 文件头与输出命名格式
